  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <LinearGradient colors={[COLORS.black, COLORS.brandDark]} style={styles.header}>
        <View style={{ flex: 1 }}><Text style={styles.title}>Dashboard</Text><Text style={styles.sub}>Mezbaan Restro Admin</Text></View>
...
/>
          ...
          <Pressable style={styles.navRow} onPress={() => router.push("/(tabs)/orders")}>
            <Ionicons name="receipt" size={22} color={COLORS.gold} />
            <Text style={styles.navTxt}>View All Orders</Text>
            <Ionicons name="chevron-forward" size={18} color={COLORS.textMuted} />
          </Pressable>
          <Pressable style={styles.navRow} onPress={() => router.push("/(tabs)/menu")}>
            <Ionicons name="restaurant" size={22} color={COLORS.gold} />
            <Text style={styles.navTxt}>Manage Menu</Text>
            <Ionicons name="chevron-forward" size={18} color={COLORS.textMuted} />
          </Pressable>
          <Pressable style={styles.navRow} onPress={() => router.push("/analytics")}>
            <Ionicons name="bar-chart" size={22} color={COLORS.gold} />
            <Text style={styles.navTxt}>Analytics</Text>
            <Ionicons name="chevron-forward" size={18} color={COLORS.textMuted} />
          </Pressable>
                  ...
                  <View style={styles.ofoot}>
                    ...
                    {isReceived ? (
                      <View style={styles.ofootActions}>
                        <Pressable onPress={() => rejectOrder(o.id)} style={styles.rejectBtn}><Ionicons name="close" size={14} color="#fff" /><Text style={styles.rejectTxt}>Reject</Text></Pressable>
                        <Pressable onPress={() => acceptOrder(o.id)} style={styles.acceptBtn}><Ionicons name="checkmark" size={14} color={COLORS.black} /><Text style={styles.acceptTxt}>Accept</Text></Pressable>
                      </View>
                    ) : next ? (
                      <Pressable onPress={() => nextStatus(o.id, o.status)} style={styles.advanceBtn}><Text style={styles.advanceTxt}>Mark {STATUS_LABEL[next]}</Text><Ionicons name="arrow-forward" size={14} color={COLORS.black} /></Pressable>
...
const styles = StyleSheet.create({
  ...
  navRow: { flexDirection: "row", alignItems: "center", margin: SPACING.lg, padding: SPACING.md, backgroundColor: COLORS.charcoal, borderRadius: RADIUS.md, gap: 12, ...SHADOW.card, borderWidth: 1, borderColor: COLORS.border },
  navTxt: { flex: 1, fontWeight: "800", color: COLORS.white },
