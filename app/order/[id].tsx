...
export default function OrderDetail() {
  ...
  const customerPhone = order?.phone || order?.user_phone || 'N/A';
  const orderItems = 
    ...
    (typeof order?.order_items === "string" ? (() => { try { return JSON.parse(order.order_items); } catch { return null; } })() : null) ||
    [];
  ...
  const acceptOrder = async () => {
    if (!order) return;
    setActing(true);
    try { await api.adminUpdateStatus(order.id, "preparing"); await stopAlert(order.id); load(); }
  ...
  const rejectOrder = () => {
    ...
    Alert.alert("Reject Order", "Are you sure?", [
      ...
      { text: "Reject", style: "destructive", onPress: async () => {
        setActing(true);
        try { await api.adminUpdateStatus(order.id, "cancelled"); await stopAlert(order.id); router.back(); }
  ...
  const advanceStatus = async () => {
    ...
    setActing(true);
    try { await api.adminUpdateStatus(order.id, STATUS_FLOW[idx + 1]); await stopAlert(order.id); load(); }
    catch { Alert.alert("Error", "Failed to update order."); }
    finally { setActing(false); }
  };

  if (loading || !order) return <View style={[styles.root, { paddingTop: insets.top }, styles.center]}><ActivityIndicator color={COLORS.gold} /></View>;
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      <LinearGradient colors={[COLORS.black, COLORS.brandDark]} style={styles.header}>
        <Pressable onPress={() => router.back()} style={styles.iconBtn}><Ionicons name="chevron-back" size={22} color="#fff" /></Pressable>
        <View style={{ flex: 1 }}><Text style={styles.title}>Order #{order.order_no}</Text><Text style={styles.sub}>{STATUS_LABEL[order.status] || order.status}</Text></View>
      </LinearGradient>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        <View style={styles.card}>
          ...
          <Text style={styles.detailText}>{order.address || 'N/A'}</Text>
      ...
      <View style={[styles.actionBar, { paddingBottom: insets.bottom + 12 }]}>
        {isReceived ? (
          <>
            <Pressable testID="order-detail-reject" onPress={rejectOrder} style={[styles.actionBtn, styles.rejectBtn]} disabled={acting}>
              <Ionicons name="close" size={20} color="#fff" /><Text style={styles.rejectTxt}>Reject</Text>
            </Pressable>
            <Pressable testID="order-detail-accept" onPress={acceptOrder} style={[styles.actionBtn, styles.acceptBtn]} disabled={acting}>
              {acting ? <ActivityIndicator color={COLORS.black} /> : <><Ionicons name="checkmark" size={20} color={COLORS.black} /><Text style={styles.acceptTxt}>Accept</Text></>}
            </Pressable>
          </>
        ) : next ? (
          <Pressable testID="order-detail-advance" onPress={advanceStatus} style={[styles.actionBtn, styles.acceptBtn]} disabled={acting}>
            {acting ? <ActivityIndicator color={COLORS.black} /> : <><Text style={styles.acceptTxt}>Mark {STATUS_LABEL[next]}</Text><Ionicons name="arrow-forward" size={20} color={COLORS.black} /></>}
          </Pressable>
        ) : (
          <View style={[styles.actionBtn, { backgroundColor: COLORS.success }]}><Ionicons name="checkmark-done" size={20} color={COLORS.black} /><Text style={styles.acceptTxt}>Order Complete</Text></View>
        )}
      </View>
    </View>
  );
