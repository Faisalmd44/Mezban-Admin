...
function getItems(order: PrintOrderData): any[] {
  ...
  if (typeof order.items === "string") {
    try { return JSON.parse(order.items); } catch {}
  }
  if (typeof order.order_items === "string") {
    try { return JSON.parse(order.order_items); } catch {}
  }
  return [];
}
...
export async function autoPrintNewOrder(order: PrintOrderData): Promise<boolean> {
