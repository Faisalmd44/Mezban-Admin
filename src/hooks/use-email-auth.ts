...
const RESET_REDIRECT_URI = makeRedirectUri({ scheme: "mezbaan", path: "reset-password" });
...
export function useEmailAuth() {
  ...
  const signIn = useCallback(async (email: string, password: string): Promise<string | null> => {
    ...
    try {
      ...
      if (sbError) { setError(sbError.message); return null; }
      if (!data.session || !data.session.access_token) { setError("Login failed — no session returned"); return null; }
      return data.session.access_token;
    } catch (e: any) { setError(e?.message || "Login failed"); return null; }
    finally { setLoading(false); }
  }, []);
  ...
  const resetPassword = useCallback(async (email: string): Promise<boolean> => {
    setError(""); setLoading(true);
    try {
      const { error: sbError } = await supabase.auth.resetPasswordForEmail(email, { redirectTo: RESET_REDIRECT_URI });
      if (sbError) { setError(sbError.message); return false; }
      return true;
    } catch (e: any) { setError(e?.message || "Failed to send reset email"); return false; }
    finally { setLoading(false); }
  }, []);
