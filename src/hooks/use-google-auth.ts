...
const REDIRECT_URI = makeRedirectUri({ scheme: "mezbaan", path: "auth/callback" });
...
export function useGoogleAuth() {
  ...
  const signIn = useCallback(async (): Promise<GoogleUser | null> => {
    ...
    try {
      ...
      if (!data?.url) { setError("No OAuth URL returned"); return null; }
      ...
      if (!accessToken) {
        ...
        if (sessError || !sessData.session) { setError("Failed to retrieve session"); return null; }
        accessToken = sessData.session.access_token;
      }

      const { data: userData, error: userError } = await supabase.auth.getUser(accessToken);
      if (userError || !userData.user) { setError("Failed to get user info"); return null; }
    ...
    } catch (e: any) { setError(e?.message || "Google sign-in failed"); return null; }
